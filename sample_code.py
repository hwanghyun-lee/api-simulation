...
#   st.sidebar.header("전체권역 조회시")
 ...
 ############################## 8.기존PPC / 후보지PPC / 기존PPC(전체data) 출력 및 서식 수정
     ...
     ############################## 9.메모테이블 수정
        ...
        memo = pd.DataFrame({
            'todo_list(~1/31)': ['polygon 내부 데이터 별도로 보여주기(해결)','주문수를 optional하게 넣을수있도록하기(복잡도 및 메모리이슈)' , 'x'  ],

            'todo_list2(~1/31)': ['코드 정규화(해결)/ 소숫점 수정(해결)' , '지도 키우기(해결)', '배포(해결(?)' ]
                       })



        st.subheader("피드백 및 향후 수정사항")
        st.table(memo.style.set_table_styles([{'selector': 'th', 'props': [('text-align', 'right')]}]))
